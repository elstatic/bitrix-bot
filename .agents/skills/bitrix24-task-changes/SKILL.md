...
3. Для операций **с подтверждением** (название, описание) — сначала `tasks.task.get`, показ текущего значения, `functions.request_user_input`
...
| Изменить название | **Да** — через `functions.request_user_input` |
| Изменить описание | **Да** — через `functions.request_user_input` |
...
**С ПОДТВЕРЖДЕНИЕМ через `functions.request_user_input`.**
...
2. Покажи пользователю текущее название и запроси подтверждение через `functions.request_user_input`:
...
2. Покажи пользователю текущее описание (или первые 200 символов, если длинное) и запроси подтверждение через `functions.request_user_input`:
