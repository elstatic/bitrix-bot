...
#### 3а. Получить список активных диалогов
...
**ВАЖНО**: `date_last_activity` показывает дату самой последней активности, а не историческую. Если чат был активен на целевой неделе, но также имел активность позже — `date_last_activity` будет показывать более позднюю дату. Поэтому **НЕ фильтруй чаты по `date_last_activity`** попадающей в целевую неделю.

Вместо этого:
1. Возьми все чаты из `result.items`
2. Исключи служебные: где `title` содержит «Уведомления», «Notifications», или `type` = `notification`
3. Исключи чаты, где `date_last_activity` **раньше** начала целевой недели (в них точно нет сообщений за неделю)
4. Из оставшихся бери **до 15 чатов** (приоритет: личные > групповые)

#### 3б. Загрузить сообщения и проверить наличие за неделю

Для каждого отобранного диалога загрузи последние сообщения:
...
- Из загруженных сообщений отфильтруй те, где `date` попадает в целевую неделю.
- **Если на первой странице нет сообщений за неделю** — пагинируй через `LAST_ID` (ID самого старого сообщения из ответа), чтобы добраться до нужного периода. Максимум **3 страницы** пагинации.
- **Если на первой странице все сообщения попадают в неделю** — загрузи ещё страницы (до 3 = 60 сообщений на диалог).
- Если после пагинации сообщений за неделю не найдено — пропускай диалог.
- Используй массив `users` из ответа для преобразования `author_id` в имена.

**Отбор для выжимки**: из диалогов, в которых нашлись сообщения за неделю, выбери **топ-10** по количеству сообщений за период.

#### 3в. Анализ и выжимка

Для каждого диалога сформируй выжимку:

1. **Тема/контекст** — о чём шла переписка (1 предложение)
2. **Ключевые договорённости и решения** — маркеры: «сделаю», «договорились», «ок, до пятницы», «принято», «давай так», «возьму на себя», «жди до...», «готово, проверь»
3. **Требует реакции** — если есть неотвеченный вопрос/просьба

**Не включай**:
- Бытовые диалоги без рабочего содержания
- Системные уведомления
- Диалоги с менее чем 2 сообщениями от разных авторов
...
Task(
  ...
  prompt: "Покажи активность в проектах за период с <WEEK_START> по <WEEK_END>",
