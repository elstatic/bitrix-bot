...
- **Проекты**: вывод субагента `project-activity-digest` вставляй как есть, без переформатирования. Если `PROJECTS_DIRS` не задан — секцию не показывай. Если за день коммитов не было — секцию не показывай.
