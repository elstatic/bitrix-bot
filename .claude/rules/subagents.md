...
**Назначение**: сканирование **всех проектов** в папках из `PROJECTS_DIRS` (`.env`) и формирование готовой сводки. Если `PROJECTS_DIRS` не задан — секция «Проекты» пропускается.
...
Если переменная отсутствует/пустая — **не вызывай** субагента.
```bash
...
**Ключевые особенности**:
- Сканирует проекты в папках из `PROJECTS_DIRS` в `.env` (если задано)
